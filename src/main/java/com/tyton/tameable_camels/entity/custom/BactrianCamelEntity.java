...
public class BactrianCamelEntity extends CamelEntity {
  ...
  protected void initDataTracker() {
    ...
    this.dataTracker.startTracking(SKIN_COLOR, CamelColorUtil.NORMAL);
  ...
  public void readCustomDataFromNbt(NbtCompound nbt) {
    ...
    if (nbt.getType("SkinColor") == NbtElement.STRING_TYPE) {
      ...
      int convertedHex = com.tyton.tameable_camels.util.CamelColorUtil.getColorFromName(colorName);
  ...
  @Override
  public BactrianCamelEntity createChild(ServerWorld serverWorld, PassiveEntity partner) {
    BactrianCamelEntity baby = ModEntities.BACTRIAN_CAMEL.create(serverWorld);

    if (baby != null && partner instanceof BactrianCamelEntity partnerCamel) {
      int parentColorA = this.getSkinColor();
      int parentColorB = partnerCamel.getSkinColor();

      int blendedChildColor = CamelColorUtil.blendParentColors(parentColorA, parentColorB);
      baby.setSkinColor(blendedChildColor);

      ServerPlayerEntity lovingPlayer = this.getLovingPlayer();
      if (lovingPlayer == null) {
        lovingPlayer = partnerCamel.getLovingPlayer();
      }
      if (lovingPlayer != null) {
        ModCriteria.BRED_COLOR_CAMEL.trigger(lovingPlayer, blendedChildColor);
      }
    }
    return baby;
  }
